...
import { Link } from 'react-router-dom'; // React Router DOM Link eklendi
import styles from '../styles/Home.module.css';

// İkonlar ve Görseller
import miniLogo from '../assets/mini-logo.png';
import { 
  FaMapMarkedAlt, FaTruck, FaUtensils, FaCalendarCheck, 
  FaChevronLeft, FaChevronRight, FaFire 
} from 'react-icons/fa';

// 1. DİZİ: HERO ALANINDAKİ 4 ADET HIT YEMEK VE INTERAKTİF SLIDE VERİSİ
const HERO_SLIDES = [
  {
    id: 1,
...
// 2. DİZİ: İlk baştaki 4 adet lüks rozet verisi (Yönlendirmeler dinamikleştirildi)
const HERO_BADGES = [
  { id: 1, icon: <FaMapMarkedAlt />, title: "АДРЕС", desc: "Наш ресторан в центре", link: "/contacts" },
  { id: 2, icon: <FaTruck />, title: "ДОСТАВКА", desc: "Премиум сервис на дом", link: "/menu" },
  { id: 3, icon: <FaUtensils />, title: "ЕДА", desc: "Шедевры от шефа", link: "/menu" },
  { id: 4, icon: <FaCalendarCheck />, title: "РЕЗЕРВ", desc: "Онлайн бронирование", link: "/booking" }
];

// 3. DİZİ: "ПОПУЛЯРНЫЕ ШЕДЕВРЫ" Bölümü İçin 10 Premium Yemek Verisi
const POPULAR_DISHES = [
  ...
  },
  {
    id: 5,
    name: "KUZU TANDIR",
    desc: "Запеченная голяшка ягненка, томившаяся 8 часов в каменной печи с ароматными травами.",
    price: 890,
    tag: "CHEF'S SPECIAL",
  ...
  },
  {
    id: 7,
    name: "LAHMACUN DUO",
    desc: "Две хрустящие турецкие мини-пиццы с пряным мясным фаршем, зеленью и лимоном.",
    price: 320,
    tag: "CRUNCHY",
...
const Home = () => {
  const aboutImg = "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=1000&auto=format&fit=crop";
  
  // Aktif slaytı kontrol eden state
  ...
  return (
    <div className={styles.homeContainer}>
      ...
      <section className={styles.heroSection}>
        ...
        <div className={styles.heroContent} key={activeSlide.id}>
          ...
          <h2 className={styles.heroSubtitle}>
            {activeSlide.id === 1 ? (
              <>ТУРЕЦКИЙ РЕСТОРАН <br /><span className={styles.goldGradientText}>НОВОГО ПОКОЛЕНИЯ</span></>
            ) : (
              <span className={styles.goldGradientText}>{activeSlide.subtitle || "ШЕДЕВР КУХНИ"}</span>
            )}
          </h2>
          ...
          <div className={styles.heroButtons}>
            {/* Butonlar Link bileşenine dönüştürüldü */}
      ...
      <section className={styles.badgesSection}>
        <div className={styles.luxuryBadgesWrapper}>
          {HERO_BADGES.map((badge) => (
            /* Tüm badge kutusu ilgili sayfaya pürüzsüz yönlendiren Link yapıldı */
            <Link to={badge.link} key={badge.id} className={styles.luxuryBadgeItem} style={{ textDecoration: 'none' }}>
      ...
      <section className={styles.aboutSection}>
        <div className={styles.aboutContainer}>
          <div className={styles.aboutImageSide}>
            ...
            <img src={aboutImg} alt="Sonsuz Luxury Interior" className={styles.aboutMainImage} />
          ...
          <div className={styles.aboutContentSide}>
            ...
            <h3 className={styles.luxuryMainTitle}>ГДЕ КАЖДОЕ БЛЮДО — <br /><span className={styles.goldText}>ИСТОРИЯ ВКУСА</span></h3>
            <p className={styles.luxuryParagraph}>
              <strong>SONSUZ</strong> — это не просто ресторан, это портал в кулинарное наследие великой Турции, переосмысленное под призмой современного хай-энд дизайна. 
            </p>
            <p className={styles.luxuryParagraph}>
              Мясо высшего качества, специи, привезенные напрямую из Стамбула, и магия открытого огня создают незабываемую симфонию.
            </p>
            {/* Buton Link bileşenine dönüştürüldü */}
      ...
      {/* 4. BÖLÜM: POPULAR DISHES */}
      <section className={styles.gurmeMenuSection}>
        <div className={styles.luxuryMenuHeader}>
          ...
          <p className={styles.menuSubDescription}>
            Эксклюзивные вкусы Анатолии, воссозданные нашими шеф-поварами в современном прочтении.
          </p>
        </div>

        <div className={styles.sliderWrapper}>
          <button className={styles.luxArrowBtn}><FaChevronLeft /></button>
            ...
            {POPULAR_DISHES.map((dish) => (
              <div key={dish.id} className={styles.gurmeCard}>
                <div className={styles.cardImageContainer}>
                  ...
                  {dish.tag && <span className={styles.cardTag}>{dish.tag}</span>}
                </div>
                <div className={styles.gurmeCardBody}>
                  ...
                  <div className={styles.gurmeCardFooter}>
                    ...
                    {/* Kart içi sipariş/detay eylemi Link yapıldı */}
          ...
          <button className={styles.luxArrowBtn}><FaChevronRight /></button>
        </div>

        <div className={styles.centerAction}>
          {/* Alt büyük menü yönlendirme butonu Link yapıldı */}
